...
$page_title = 'Inspirations Coiffures - Admin HairRoots';
$message = '';
$message_type = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        $message = "Nouveau modele ajoute avec succes.";
    }
    $message_type = "success";
}
...
<style>
:root {
    --gold:#C9A84C; --orange:#C1622F; --dark:#3E1F0D;
    --medium:#6B3A2A; --light:#F5E6D3; --cream:#FDF8F2; --white:#FFFFFF;
}
.stats-bar{display:flex;gap:15px;margin-bottom:25px;flex-wrap:wrap}
.stat-pill{background:var(--white);border:1px solid var(--light);border-radius:14px;padding:12px 22px;display:flex;align-items:center;gap:10px;font-size:0.88rem;font-weight:600;color:var(--dark);box-shadow:0 2px 8px rgba(62,31,13,0.06)}
.stat-pill .num{font-size:1.4rem;font-weight:800;color:var(--orange)}
.table-card{background:var(--white);border-radius:20px;border:1px solid var(--light);box-shadow:0 4px 20px rgba(62,31,13,0.06);overflow:hidden;margin-bottom:30px}
.table-card-header{background:linear-gradient(135deg,#FDF3E7,#FBE8D0);padding:18px 24px;border-bottom:1px solid var(--light);display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
.table-card-header h5{font-family:'Playfair Display',serif;color:var(--dark);font-weight:700;margin:0;font-size:1.1rem}
.table-responsive{overflow-x:auto}
table.coiff-table{width:100%;border-collapse:collapse}
table.coiff-table th{padding:12px 16px;font-size:0.78rem;font-weight:700;color:var(--medium);text-transform:uppercase;letter-spacing:0.05em;border-bottom:2px solid var(--light);white-space:nowrap;background:linear-gradient(90deg,#FDF3E7,#FBE8D0)}
table.coiff-table td{padding:14px 16px;border-bottom:1px solid #FDF3E7;color:var(--dark);font-size:0.88rem;vertical-align:middle}
table.coiff-table tr:last-child td{border-bottom:none}
table.coiff-table tr:hover td{background:#FFFDF9}
.coiff-thumb{width:52px;height:52px;border-radius:12px;object-fit:cover;object-position:top;border:2px solid var(--light)}
.coiff-thumb-ph{width:52px;height:52px;border-radius:12px;background:linear-gradient(135deg,var(--light),#FDEBD0);display:flex;align-items:center;justify-content:center;font-size:1.5rem;border:2px solid var(--light)}
...
.badge-tendance{background:linear-gradient(135deg,var(--gold),#b8942e);color:var(--dark);padding:3px 10px;border-radius:8px;font-size:0.72rem;font-weight:700}
.btn-edit{background:linear-gradient(135deg,var(--gold),#b8942e);color:var(--dark);border:none;border-radius:9px;padding:7px 14px;font-size:0.8rem;font-weight:700;text-decoration:none;display:inline-flex;align-items:center;gap:5px;transition:all 0.2s}
.btn-edit:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(201,168,76,0.4);color:var(--dark)}
.btn-delete{background:#FCE4E4;color:#C62828;border:none;border-radius:9px;padding:7px 14px;font-size:0.8rem;font-weight:700;display:inline-flex;align-items:center;gap:5px;transition:all 0.2s;cursor:pointer}
.btn-delete:hover{background:#C62828;color:#fff;transform:translateY(-1px)}
.btn-add{background:linear-gradient(135deg,var(--orange),#a0491f);color:#fff;border:none;border-radius:12px;padding:11px 24px;font-size:0.88rem;font-weight:700;display:inline-flex;align-items:center;gap:8px;text-decoration:none;transition:all 0.3s;cursor:pointer}
.btn-add:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(193,98,47,0.35);color:#fff}
.form-card{background:var(--white);border-radius:20px;border:2px solid var(--light);box-shadow:0 6px 30px rgba(62,31,13,0.08);overflow:hidden;margin-bottom:30px}
.form-card-header{background:linear-gradient(135deg,var(--dark),var(--medium));padding:20px 28px;display:flex;align-items:center;justify-content:space-between}
.form-card-header h5{font-family:'Playfair Display',serif;color:#fff;margin:0;font-size:1.15rem}
.form-card-body{padding:28px}
.form-label-custom{font-weight:700;font-size:0.82rem;color:var(--medium);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:6px;display:block}
.form-control-custom{width:100%;padding:11px 15px;border:2px solid var(--light);border-radius:12px;background:var(--cream);font-size:0.9rem;color:var(--dark);outline:none;transition:border-color 0.2s;font-family:'Poppins',sans-serif}
.form-control-custom:focus{border-color:var(--gold);background:#fff}
select.form-control-custom{cursor:pointer}
.toggle-wrap{display:flex;align-items:center;gap:12px}
.toggle-input{display:none}
.toggle-label{width:50px;height:26px;background:#ddd;border-radius:13px;position:relative;cursor:pointer;transition:background 0.3s}
.toggle-label::after{content:'';position:absolute;width:20px;height:20px;border-radius:50%;background:#fff;top:3px;left:3px;transition:left 0.3s;box-shadow:0 1px 4px rgba(0,0,0,0.2)}
...
.photo-preview{width:90px;height:90px;border-radius:14px;object-fit:cover;object-position:top;border:3px solid var(--light);display:none}
.photo-preview.show{display:block}
.photo-ph{width:90px;height:90px;border-radius:14px;background:linear-gradient(135deg,var(--light),#FDEBD0);display:flex;align-items:center;justify-content:center;font-size:2.5rem;border:3px solid var(--light)}
.alert-custom{border-radius:14px;padding:14px 20px;font-weight:600;font-size:0.9rem;display:flex;align-items:center;gap:10px;margin-bottom:22px}
.alert-success{background:#E8F5E9;color:#2E7D32;border:1px solid #C8E6C9}
.alert-error{background:#FCE4E4;color:#C62828;border:1px solid #FFCDD2}
.empty-state{text-align:center;padding:50px 20px;color:var(--medium)}
...
<div class="container-fluid py-4 px-4">
    ...
    <div class="table-card">
        <div class="table-card-header">
            <h5><i class="bi bi-grid-3x3-gap me-2" style="color:var(--gold)"></i>Liste des modeles</h5>
        ...
        <div class="table-responsive">
            ...
            <table class="coiff-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge-<?= strtolower($m['difficulte']) ?>"><?= htmlspecialchars($m['difficulte']) ?></span></td>
                        ...
                        <td>
                            <div style="display:flex;gap:6px;flex-wrap:wrap">
                                ...
                                <button class="btn-delete"
                                    onclick="if(confirm('Supprimer ce modele ?')) window.location='?delete=<?= $m['id'] ?>'">
            ...
            <div class="empty-state">
                <h5>Aucun modele pour le moment</h5>
                <p>Ajoutez votre premier modele de coiffure ci-dessous.</p>
            </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="form-card" id="form-section">
        <div class="form-card-header">
            <h5>
                ...
                <?= $edit_modele ? 'Modifier : ' . htmlspecialchars($edit_modele['nom']) : 'Ajouter un nouveau modele' ?>
            </h5>
            <?php if ($edit_modele): ?>
                ...
                <div class="row g-4">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="nom" class="form-control-custom" required
                            placeholder="Ex: Twist Out Boucle"
                            value="<?= htmlspecialchars($edit_modele['nom'] ?? '') ?>">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label-custom">Prix a partir de (€) *</label>
                    ...
                    <div class="col-md-3">
                        <label class="form-label-custom">Duree de realisation *</label>
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="description" class="form-control-custom" rows="3"
                            placeholder="Decrivez le style..."><?= htmlspecialchars($edit_modele['description'] ?? '') ?></textarea>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label-custom">Type de cheveux *</label>
                        <select name="type_cheveux" class="form-control-custom" required>
                            <option value="">— Choisir —</option>
                            <?php foreach (['Boucles','Crepus','Lisses','Ondules'] as $t): ?>
                    ...
                    <div class="col-md-4">
                        <label class="form-label-custom">Difficulte *</label>
                    ...
                    <div class="col-md-6">
                        ...
                        <div style="display:flex;align-items:center;gap:15px">
                            ...
                            <div style="flex:1">
                                ...
                                <div style="color:#9a7c5c;font-size:0.75rem;margin-top:5px">JPG, PNG, WEBP — Max 2 Mo</div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div style="color:#9a7c5c;font-size:0.78rem;margin-top:8px">
                            Les 4 premieres tendances sont affichees sur la page publique.
                        </div>
                    </div>
                    <div class="col-12" style="padding-top:10px;border-top:1px solid var(--light);display:flex;gap:12px;flex-wrap:wrap">
                        <button type="submit" class="btn-add" style="padding:13px 30px">
                            ...
                            <?= $edit_modele ? 'Enregistrer les modifications' : 'Ajouter le modele' ?>
                        </button>
                        <?php if ($edit_modele): ?>
