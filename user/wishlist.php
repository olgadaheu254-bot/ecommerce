...
<div class="container">
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:25px;flex-wrap:wrap;gap:10px">
        ...
        <button onclick="ajouterToutPanier()" style="background:linear-gradient(135deg,#C9A84C,#b8942e);color:#3E1F0D;border:none;border-radius:12px;padding:10px 22px;font-weight:700;font-size:0.88rem;cursor:pointer;">
        ...
        <div class="col-lg-3 col-md-4 col-sm-6" id="wish-item-<?= $p['id'] ?>">
            <div class="wish-card">
                ...
                <div class="wish-body">
                    ...
                    <?php endif; ?>
                    <a href="/ecommerce/products/detail.php?id=<?= $p['id'] ?>" style="display:block;text-align:center;text-decoration:none;background:#F5E6D3;color:#3E1F0D;border-radius:10px;padding:8px;font-size:0.8rem;font-weight:600;">
                         Voir le produit
                    </a>
                </div>
            </div>
    ...
    <div class="empty-wish">
        ...
        <p style="color:#9a7c5c;margin:10px 0 30px">Ajoutez des produits à vos favoris en cliquant sur le coeur</p>
...
function ajouterPanier(productId) {
    ...
    .then(r => r.json())
    .then(data => {
        if(data.success) showToast(' Ajouté au panier !');
...
function ajouterToutPanier() {
    ...
    items.forEach(item => {
        ...
        if(btn && !btn.disabled) {
            ...
            }).then(()=>{ count++; if(count===items.length) showToast(' Tous les articles ajoutés au panier !'); });
        }
    });
}
</script>
